...
namespace Akgul_Yemek.Controllers
{
    public class Admin_SistemController : Controller
    {
        ...
        public ActionResult OnKayit_Listele(string id)
        {
            ...
            {
                case 0:
                    ViewData["orgs"] = today_org_infos.OrderBy(w => w.date).ToList();
                    break;
                case 1:
                    ViewData["orgs"] = future_org_infos.OrderBy(w => w.date).ToList();
                    break;
                case 2:
                    ViewData["orgs"] = finished_org_infos.OrderByDescending(w => w.date).ToList();
                    break;
                case 3:
                    ViewData["orgs"] = nonAccept_org_infos.OrderBy(w => w.date).ToList();
                    break;
                case 4:
                    ViewData["orgs"] = completely_finished_org_infos.OrderByDescending(w => w.date).ToList();
                    break;
        ...
        public ActionResult Transition_RaporAl_PersonelRaporu(ReportModel reportModel)
        {
            try
            {
                ...
                akgul_yemek_dbEntities db = new akgul_yemek_dbEntities();
                List<organization_information> orgInfos = db.organization_information.Where(w => w.date >= reportModel.startDate && w.date <= reportModel.stopDate).OrderByDescending(w => w.date).ToList();
